...
fn validateCommand(
    ...
    depth: usize,
    globals: *[max_globals]*const Option,
    ...
    for (command.options, 0..) |*current, index| {
        ...
        if (current.default_value) |default_value| {
            if (!options.validValue(current, default_value)) {
                return error.InvalidOptionDefault;
            }
        }

        for (command.options[0..index]) |previous| {
            if (std.mem.eql(u8, current.long, previous.long)) {
                return error.DuplicateOptionName;
            }
            if (current.short != null and current.short == previous.short) {
                return error.DuplicateShortOption;
            }
        }

        for (globals[0..inherited_count]) |global| {
            if (std.mem.eql(u8, current.long, global.long)) {
                return error.DuplicateOptionName;
            }
            if (current.short != null and current.short == global.short) {
                return error.DuplicateShortOption;
            }
        }

        for (current.requires) |name| {
            if (!optionExists(command.options, globals[0..inherited_count], name)) {
                return error.UnknownOptionReference;
            }
        }
        for (current.conflicts_with) |name| {
            if (!optionExists(command.options, globals[0..inherited_count], name)) {
                return error.UnknownOptionReference;
            }
        }
    ...
    for (command.args.positionals, 0..) |current, index| {
        ...
        for (command.args.positionals[0..index]) |previous| {
            if (std.mem.eql(u8, current.name, previous.name)) {
                return error.DuplicatePositionalName;
            }
        }
        if (!current.required) saw_optional = true;
        if (current.required and saw_optional) {
            return error.RequiredPositionalAfterOptional;
        }
...
fn optionExists(
    local: []const Option,
    inherited: []const *const Option,
    name: []const u8,
) bool {
    for (local) |definition| {
        if (std.mem.eql(u8, definition.long, name)) return true;
    }
