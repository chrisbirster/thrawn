...
pub fn writeCandidates(allocator: std.mem.Allocator, writer: *std.Io.Writer, root: *const Command, words: []const []const u8) !void {
    ...
    while (index < committed.len) {
        ...
        if (std.mem.eql(u8, token, "--")) { positional_start = index + 1; break; }
        if (token.len > 0 and token[0] == '-') {
            if (consumeOption(root, current, committed, &index)) { index += 1; continue; }
            positional_start = index;
            break;
        }
        if (current.findChild(token)) |child| { current = child; index += 1; continue; }
        positional_start = index;
        break;
    }
    ...
    if (current.complete) |complete| {
        const positional_args = if (positional_start <= committed.len) committed[positional_start..] else &.{};
        var context: command_mod.CompletionContext = .{ .allocator = allocator, .args = positional_args, .prefix = prefix, .writer = writer };
        try complete(&context);
    }
}

fn consumeOption(root: *const Command, command: *const Command, words: []const []const u8, index: *usize) bool {
    ...
    if (token.len > 2 and std.mem.startsWith(u8, token, "--")) {
        ...
        if (definition.kind == .value and equals_index == null and index.* + 1 < words.len) index.* += 1;
        return true;
    }
    if (token.len > 1 and token[0] == '-' and token[1] != '-') {
        ...
        while (short_index < token.len) : (short_index += 1) {
            ...
            if (definition.kind == .value) {
                if (short_index + 1 == token.len and index.* + 1 < words.len) index.* += 1;
                break;
            }
        }
        return true;
...
fn writeOptionCandidates(writer: *std.Io.Writer, root: *const Command, command: *const Command, prefix: []const u8) !void {
    for (command.options) |*definition| try writeDefinition(writer, definition, prefix);
...
fn writeDefinition(writer: *std.Io.Writer, definition: *const Option, prefix: []const u8) !void {
...
fn findEffectiveLong(root: *const Command, command: *const Command, name: []const u8) ?*const Option {
    ...
    while (index > 0) {
        index -= 1;
        if (options.findLong(commands[index].options, name)) |definition| if (definition.global) return definition;
    }
    return null;
}

fn findEffectiveShort(root: *const Command, command: *const Command, short: u8) ?*const Option {
    ...
    while (index > 0) {
        index -= 1;
        if (options.findShort(commands[index].options, short)) |definition| if (definition.global) return definition;
    }
    return null;
}
