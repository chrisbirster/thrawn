...
pub fn write(
    ...
    if (command.usage) |usage| {
        ...
        if (command.options.len > 0 or inheritedGlobalCount(root, command) > 0) {
    ...
    if (command.options.len > 0) {
        try writeOptions(writer, "Options", command.options, false);
    }
...
fn writeChildren(
    ...
    for (command.children) |child| {
        if (!child.hidden and child.group == null) {
            ungrouped = true;
            break;
        }
    }
    ...
    for (command.children, 0..) |child, index| {
        ...
        for (command.children[0..index]) |previous| {
            if (previous.group) |previous_group| {
                if (std.mem.eql(u8, group, previous_group)) {
                    seen = true;
                    break;
                }
            }
        ...
        for (command.children) |grouped| {
            ...
            if (grouped.group) |grouped_name| {
                if (std.mem.eql(u8, group, grouped_name)) {
...
fn writeOptions(
    ...
    for (definitions) |definition| {
        if (!globals_only or definition.global) count += 1;
    }
    if (count == 0) return;
    ...
    for (definitions) |definition| {
        ...
        if (definition.short) |short| {
            try writer.print("  -{c}, --{s}", .{ short, definition.long });
        } else {
...
fn inheritedGlobalCount(root: *const Command, command: *const Command) usize {
    ...
    for (commands[0 .. len - 1]) |ancestor| {
        for (ancestor.options) |definition| {
            if (definition.global) count += 1;
        }
    }
    return count;
}

fn writeInheritedGlobals(
    writer: *std.Io.Writer,
    root: *const Command,
    command: *const Command,
) std.Io.Writer.Error!void {
    ...
    for (commands[0 .. len - 1]) |ancestor| {
        for (ancestor.options) |definition| {
            ...
            if (definition.kind == .value) {
                try writer.print(" <{s}>", .{definition.value_name orelse "value"});
            }
            if (definition.summary.len > 0) {
                try writer.print("\n      {s}", .{definition.summary});
            }
